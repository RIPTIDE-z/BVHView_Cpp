...
char* ArgFind(int argc, char** argv, const char* name)
{
    ...
    {
        if (strlen(argv[i]) > 4 && argv[i][0] == '-' && argv[i][1] == '-' && strstr(argv[i] + 2, name) == argv[i] + 2)
        {
...
Color ArgColor(int argc, char** argv, const char* name, Color defaultValue)
{
    ...
    {
        printf("INFO: Parsed option '%s' as '%s'\n", name, value);
        return Color{static_cast<unsigned char>(ClampInt(cx, 0, 255)), static_cast<unsigned char>(ClampInt(cy, 0, 255)),
                     static_cast<unsigned char>(ClampInt(cz, 0, 255)), 255};
    }
