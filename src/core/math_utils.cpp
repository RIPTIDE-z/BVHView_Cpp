// Implements common math and view-frustum calculations
...
Quaternion QuaternionBetween(Vector3 p, Vector3 q)
{
    ...
    return QuaternionLength(o) < 1e-8f ? QuaternionFromAxisAngle(Vector3{1.0f, 0.0f, 0.0f}, PI)
...
Quaternion QuaternionHermite(Quaternion r0, Quaternion r1, Vector3 v0, Vector3 v1, float alpha)
{
    ...
    return QuaternionMultiply(QuaternionFromScaledAngleAxis(Vector3Add(
                                  Vector3Add(Vector3Scale(r1r0, w1), Vector3Scale(v0, w2)), Vector3Scale(v1, w3))),
                              r0);
}
...
Frustum FrustumFromCameraMatrices(Matrix projection, Matrix modelview)
{
    ...
    Frustum frustum;
    frustum.back = FrustumPlaneNormalize(
        Vector4{planes.m3 - planes.m2, planes.m7 - planes.m6, planes.m11 - planes.m10, planes.m15 - planes.m14});
    frustum.front = FrustumPlaneNormalize(
        Vector4{planes.m3 + planes.m2, planes.m7 + planes.m6, planes.m11 + planes.m10, planes.m15 + planes.m14});
    frustum.bottom = FrustumPlaneNormalize(
        Vector4{planes.m3 + planes.m1, planes.m7 + planes.m5, planes.m11 + planes.m9, planes.m15 + planes.m13});
    frustum.top = FrustumPlaneNormalize(
        Vector4{planes.m3 - planes.m1, planes.m7 - planes.m5, planes.m11 - planes.m9, planes.m15 - planes.m13});
    frustum.left = FrustumPlaneNormalize(
        Vector4{planes.m3 + planes.m0, planes.m7 + planes.m4, planes.m11 + planes.m8, planes.m15 + planes.m12});
    frustum.right = FrustumPlaneNormalize(
        Vector4{planes.m3 - planes.m0, planes.m7 - planes.m4, planes.m11 - planes.m8, planes.m15 - planes.m12});
    return frustum;
}
...
{
    if (FrustumPlaneDistanceToPoint(frustum.back, position) < -radius)
    {
        return false;
    }
    if (FrustumPlaneDistanceToPoint(frustum.front, position) < -radius)
    {
        return false;
    }
    if (FrustumPlaneDistanceToPoint(frustum.bottom, position) < -radius)
    {
        return false;
    }
    if (FrustumPlaneDistanceToPoint(frustum.top, position) < -radius)
    {
        return false;
    }
    if (FrustumPlaneDistanceToPoint(frustum.left, position) < -radius)
    {
        return false;
    }
    if (FrustumPlaneDistanceToPoint(frustum.right, position) < -radius)
    {
        return false;
    }
    return true;
