...
void GuiOrbitCamera(OrbitCamera* camera, CharacterData* characterData, int argc, char** argv)
{
    ...
    GuiLabel(UiRectangle(30, 80, 225, 20), TextFormat("Target: [% 5.3f % 5.3f % 5.3f]", camera->cam3d.target.x,
                                                      camera->cam3d.target.y, camera->cam3d.target.z));
    GuiLabel(UiRectangle(30, 100, 225, 20),
             TextFormat("Offset: [% 5.3f % 5.3f % 5.3f]", camera->offset.x, camera->offset.y, camera->offset.z));
    ...
    if (characterData->count > 0)
    {
        ...
        UiComboBox(UiRectangle(30, 240, 150, 20), characterData->jointNamesCombo[characterData->active].c_str(),
                   &camera->trackBone);
    }
}
...
void GuiRenderSettings(RenderSettings* settings, CapsuleData* capsuleData, int windowWidth, int)
{
    ...
    GuiSliderBar(UiRectangle(screenWidth - 160, 20, 100, 20), "Exposure", TextFormat("%5.2f", settings->exposure),
                 &settings->exposure, 0.0f, 3.0f);

    GuiSliderBar(UiRectangle(screenWidth - 160, 50, 100, 20), "Sun Light",
                 TextFormat("%5.2f", settings->sunLightStrength), &settings->sunLightStrength, 0.0f, 1.0f);

    if (GuiSliderBar(UiRectangle(screenWidth - 160, 80, 100, 20), "Sun Softness",
                     TextFormat("%5.2f", settings->sunLightConeAngle), &settings->sunLightConeAngle, 0.02f, PI / 4.0f))
    {
        CapsuleDataUpdateShadowLookupTable(capsuleData, settings->sunLightConeAngle);
    }

    GuiSliderBar(UiRectangle(screenWidth - 160, 110, 100, 20), "Sky Light",
                 TextFormat("%5.2f", settings->skyLightStrength), &settings->skyLightStrength, 0.0f, 1.0f);

    GuiSliderBar(UiRectangle(screenWidth - 160, 140, 100, 20), "Ambient Light",
                 TextFormat("%5.2f", settings->ambientLightStrength), &settings->ambientLightStrength, 0.0f, 2.0f);

    GuiSliderBar(UiRectangle(screenWidth - 160, 170, 100, 20), "Ground Light",
                 TextFormat("%5.2f", settings->groundLightStrength), &settings->groundLightStrength, 0.0f, 0.5f);

    GuiSliderBar(UiRectangle(screenWidth - 160, 200, 100, 20), "Sun Azimuth", TextFormat("%5.2f", settings->sunAzimuth),
                 &settings->sunAzimuth, -PI, PI);

    GuiSliderBar(UiRectangle(screenWidth - 160, 230, 100, 20), "Sun Altitude",
                 TextFormat("%5.2f", settings->sunAltitude), &settings->sunAltitude, 0.0f, 0.49f * PI);
                      ...
                      ScrubberSettings* scrubberSettings, char* errMsg, int argc, char** argv)
{
    ...
    for (int i = 0; i < characterData->count; i++)
    {
        ...
        if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && CheckCollisionPointRec(GetMousePosition(), colorBounds))
        {
    ...
    if (characterData->count > 0)
    {
        ...
        if (scaleM)
        {
            characterData->scales[characterData->active] = 1.0f;
        }
        ...
        if (scaleCM)
        {
            characterData->scales[characterData->active] = 0.01f;
        }
        ...
        if (scaleInches)
        {
            characterData->scales[characterData->active] = 0.0254f;
        }
        ...
        if (scaleFeet)
        {
            characterData->scales[characterData->active] = 0.3048f;
        }

        bool scaleAuto =
            characterData->scales[characterData->active] == characterData->autoScales[characterData->active];
        ...
        GuiSliderBar(UiRectangle(70, offsetHeight + 90 + (CHARACTERS_MAX - 1) * 30, 100, 20), "Radius",
                     TextFormat("%5.2f", characterData->radii[characterData->active]),
                     &characterData->radii[characterData->active], 0.01f, 0.1f);

        GuiSliderBar(UiRectangle(70, offsetHeight + 120 + (CHARACTERS_MAX - 1) * 30, 100, 20), "Opacity",
                     TextFormat("%5.2f", characterData->opacities[characterData->active]),
                     &characterData->opacities[characterData->active], 0.0f, 1.0f);
    }
}

void GuiScrubberSettings(ScrubberSettings* settings, CharacterData* characterData, int windowWidth, int windowHeight)
{
    if (characterData->count == 0)
    {
        return;
    ...
    UiComboBox(UiRectangle(screenWidth / 2 - 240, screenHeight - 80, 100, 20), "Nearest;Linear;Cubic",
               &settings->sampleMode);
    ...
    GuiToggle(UiRectangle(screenWidth / 2 + 50, screenHeight - 80, 30, 20), "0.1x", &speed01x);
    if (speed01x)
    {
        settings->playSpeed = 0.1f;
    }
    bool speed05x = settings->playSpeed == 0.5f;
    GuiToggle(UiRectangle(screenWidth / 2 + 90, screenHeight - 80, 30, 20), "0.5x", &speed05x);
    if (speed05x)
    {
        settings->playSpeed = 0.5f;
    }
    bool speed1x = settings->playSpeed == 1.0f;
    GuiToggle(UiRectangle(screenWidth / 2 + 130, screenHeight - 80, 30, 20), "1x", &speed1x);
    if (speed1x)
    {
        settings->playSpeed = 1.0f;
    }
    bool speed2x = settings->playSpeed == 2.0f;
    GuiToggle(UiRectangle(screenWidth / 2 + 170, screenHeight - 80, 30, 20), "2x", &speed2x);
    if (speed2x)
    {
        settings->playSpeed = 2.0f;
    }
    bool speed4x = settings->playSpeed == 4.0f;
    GuiToggle(UiRectangle(screenWidth / 2 + 210, screenHeight - 80, 30, 20), "4x", &speed4x);
    if (speed4x)
    {
        settings->playSpeed = 4.0f;
    }
    GuiSliderBar(UiRectangle(screenWidth / 2 + 250, screenHeight - 80, 70, 20), "",
                 TextFormat("%5.2fx", settings->playSpeed), &settings->playSpeed, 0.0f, 4.0f);

    int frame = ClampInt((int)(settings->playTime / frameTime + 0.5f), settings->frameMin, settings->frameMax);

    if (GuiValueBox(UiRectangle(screenWidth / 2 - 540, screenHeight - 80, 50, 20), "Min   ", &settings->frameMinSelect,
                    0, settings->frameLimit, settings->frameMinEdit))
    {
    ...
    if (GuiValueBox(UiRectangle(screenWidth / 2 + 470, screenHeight - 80, 50, 20), "Max   ", &settings->frameMaxSelect,
                    0, settings->frameLimit, settings->frameMaxEdit))
    {
    ...
    GuiLabel(UiRectangle(screenWidth / 2 + 530, screenHeight - 80, 100, 20), TextFormat("of %i", settings->frameLimit));

    float frameFloatPrev = settings->frameSnap ? (float)frame : settings->playTime / frameTime;
    float frameFloat = frameFloatPrev;

    GuiSliderBar(UiRectangle(screenWidth / 2 - 540, screenHeight - 50, 1080, 20),
                 TextFormat("%5.2f", settings->playTime), TextFormat("%i", frame), &frameFloat,
