 ...
 *       #include "gui_window_file_dialog.hpp"
*
 ...
 *
 *   ALTERED: Adapted for strict C++17 compilation in BVHView_Cpp
 *
...
GuiWindowFileDialogState InitGuiWindowFileDialog(const char* initPath)
{
    ...
    // Init window data
    state.windowBounds = Rectangle{static_cast<float>(GetScreenWidth() / 2 - 440 / 2),
                                   static_cast<float>(GetScreenHeight() / 2 - 310 / 2), 440.0f, 310.0f};
...
void GuiWindowFileDialog(GuiWindowFileDialogState* state)
{
    if (state->windowActive)
    {
        ...
        if (GuiButton(Rectangle{state->windowBounds.x + state->windowBounds.width - 48, state->windowBounds.y + 24 + 12,
                                40, 24},
                      "< .."))
        {
        ...
        if (GuiTextBox(Rectangle{state->windowBounds.x + 8, state->windowBounds.y + 24 + 12,
        ...
        state->filesListActive = GuiListViewFiles(
            Rectangle{state->position.x + 8, state->position.y + 48 + 20, state->windowBounds.width - 16,
                      state->windowBounds.height - 60 - 16 - 68},
            fileInfo, state->dirFiles.count, &state->itemFocused, &state->filesListScrollIndex, state->filesListActive);
#else
        GuiListViewEx(Rectangle{state->windowBounds.x + 8, state->windowBounds.y + 48 + 20,
                                state->windowBounds.width - 16, state->windowBounds.height - 60 - 16 - 68},
                      (char**)dirFilesIcon, state->dirFiles.count, &state->filesListScrollIndex,
                      &state->filesListActive, &state->itemFocused);
#endif
            ...
            if (DirectoryExists(TextFormat("%s/%s", state->dirPathText, state->fileNameText)))
            {
                if (TextIsEqual(state->fileNameText, ".."))
                    ...
                    strcpy(state->dirPathText,
                           TextFormat("%s/%s", (strcmp(state->dirPathText, "/") == 0) ? "" : state->dirPathText,
                                      state->fileNameText));
        ...
        //--------------------------------------------------------------------------------------
        GuiLabel(Rectangle{state->windowBounds.x + 8, state->windowBounds.y + state->windowBounds.height - 68, 60, 24},
                 "File name:");
        if (GuiTextBox(Rectangle{state->windowBounds.x + 72, state->windowBounds.y + state->windowBounds.height - 68,
        ...
        GuiLabel(
            Rectangle{state->windowBounds.x + 8, state->windowBounds.y + state->windowBounds.height - 24 - 12, 68, 24},
            "File filter:");
        GuiComboBox(Rectangle{state->windowBounds.x + 72, state->windowBounds.y + state->windowBounds.height - 24 - 12,
                              state->windowBounds.width - 184, 24},
                    "All files", &state->fileTypeActive);

        state->SelectFilePressed = GuiButton(Rectangle{state->windowBounds.x + state->windowBounds.width - 96 - 8,
                                                       state->windowBounds.y + state->windowBounds.height - 68, 96, 24},
                                             "Select");

        if (GuiButton(Rectangle{state->windowBounds.x + state->windowBounds.width - 96 - 8,
                                state->windowBounds.y + state->windowBounds.height - 24 - 12, 96, 24},
                      "Cancel"))
            state->windowActive = false;
        //--------------------------------------------------------------------------------------
...
static void ReloadDirectoryFiles(GuiWindowFileDialogState* state)
{
    ...
    state->dirFiles = LoadDirectoryFilesEx(
        state->dirPathText, (state->filterExt[0] == '\0') ? "*.*" : TextFormat("DIRS*;%s", state->filterExt), false);
    state->itemFocused = 0;
    ...
    for (unsigned int i = 0; i < state->dirFiles.count; i++)
    {
        if (IsPathFile(state->dirFiles.paths[i]))
        {
            ...
            else if (IsFileExtension(state->dirFiles.paths[i], ".exe;.bin;.raw;.msi"))
            {
                ...
                strcpy(dirFilesIcon[i], TextFormat("#218#%s", GetFileName(state->dirFiles.paths[i])));
        }
        else
        {
...
static int GuiListViewFiles(Rectangle bounds, FileInfo* files, int count, int* focus, int* scrollIndex, int* active)
{
    ...
    itemBounds.width =
        bounds.width - 2 * GuiGetStyle(LISTVIEW, LIST_ITEMS_PADDING) - GuiGetStyle(DEFAULT, BORDER_WIDTH);
    ...
    int visibleItems =
        bounds.height / (GuiGetStyle(LISTVIEW, LIST_ITEMS_HEIGHT) + GuiGetStyle(LISTVIEW, LIST_ITEMS_PADDING));
    ...
    DrawRectangleLinesEx(bounds, GuiGetStyle(DEFAULT, BORDER_WIDTH),
    ...
    for (int i = 0; i < visibleItems; i++)
    {
        if (state == GUI_STATE_DISABLED)
        {
            if ((startIndex + i) == itemSelected)
            {
                ...
                DrawRectangleLinesEx(itemBounds, GuiGetStyle(LISTVIEW, BORDER_WIDTH),
            ...
            GuiDrawText(files[startIndex + i].name, GetTextBounds(DEFAULT, itemBounds),
