                                     ...
                                     Vector3 line1Start, Vector3 line1End)
{
    ...
    *nearestTime1 =
        NearestPointOnLineSegment(line1Start, line1Vec, Vector3Add(line0Start, Vector3Scale(line0Vec, *nearestTime0)));
    *nearestTime0 =
        NearestPointOnLineSegment(line0Start, line0Vec, Vector3Add(line1Start, Vector3Scale(line1Vec, *nearestTime1)));
}
...
void NearestPointBetweenLineSegmentAndGroundSegment(float* nearestTimeOnLine, Vector3* nearestPointOnGround,
                                                    Vector3 lineStart, Vector3 lineEnd, Vector3 groundMins,
                                                    Vector3 groundMaxs)
{
    ...
    *nearestPointOnGround = Vector3{
    ...
    if (nearestPointOnGround->x >= groundMins.x && nearestPointOnGround->x <= groundMaxs.x &&
        nearestPointOnGround->z >= groundMins.z && nearestPointOnGround->z <= groundMaxs.z)
    {
        return;
    }
    ...
    NearestPointBetweenLineSegments(&nearestTimeOnLine0, &nearestTimeOnEdge0, lineStart, lineEnd, edgeStart0, edgeEnd0);

    NearestPointBetweenLineSegments(&nearestTimeOnLine1, &nearestTimeOnEdge1, lineStart, lineEnd, edgeStart1, edgeEnd1);

    NearestPointBetweenLineSegments(&nearestTimeOnLine2, &nearestTimeOnEdge2, lineStart, lineEnd, edgeStart2, edgeEnd2);

    NearestPointBetweenLineSegments(&nearestTimeOnLine3, &nearestTimeOnEdge3, lineStart, lineEnd, edgeStart3, edgeEnd3);
    ...
    Vector3 nearestPointOnEdge0 =
        Vector3Add(edgeStart0, Vector3Scale(Vector3Subtract(edgeEnd0, edgeStart0), nearestTimeOnEdge0));
    Vector3 nearestPointOnEdge1 =
        Vector3Add(edgeStart1, Vector3Scale(Vector3Subtract(edgeEnd1, edgeStart1), nearestTimeOnEdge1));
    Vector3 nearestPointOnEdge2 =
        Vector3Add(edgeStart2, Vector3Scale(Vector3Subtract(edgeEnd2, edgeStart2), nearestTimeOnEdge2));
    Vector3 nearestPointOnEdge3 =
        Vector3Add(edgeStart3, Vector3Scale(Vector3Subtract(edgeEnd3, edgeStart3), nearestTimeOnEdge3));
...
void NearestPointBetweenLineSegmentAndSweptLine(float* nearestTimeOnLine, Vector3* nearestPointOnSweptLine,
                                                Vector3 lineStart, Vector3 lineEnd, Vector3 sweptLineStart,
                                                Vector3 sweptLineEnd, Vector3 sweptLineSweepVector)
{
    Vector3 lineVec = Vector3Subtract(lineEnd, lineStart);
    ...
    {
        nearestPointOnSweptLine0 =
            ProjectPointOntoSweptLine(sweptLineStart, sweptLineVec, sweptLineSweepVector, nearestPointOnLine0);
    }
    else
    {
        float nearestTimeOnSweptLine =
            NearestPointOnLineSegment(sweptLineStart, sweptLineSweepVector, nearestPointOnLine0);

        nearestPointOnSweptLine0 =
            Vector3Add(sweptLineStart, Vector3Scale(sweptLineSweepVector, nearestTimeOnSweptLine));
    }
    ...
    Vector3 nearestPointOnSweptLine1 =
        Vector3Add(edgeStart1, Vector3Scale(Vector3Subtract(edgeEnd1, edgeStart1), nearestTimeOnEdge1));
    Vector3 nearestPointOnSweptLine2 =
        Vector3Add(edgeStart2, Vector3Scale(Vector3Subtract(edgeEnd2, edgeStart2), nearestTimeOnEdge2));
    Vector3 nearestPointOnSweptLine3 =
        Vector3Add(edgeStart3, Vector3Scale(Vector3Subtract(edgeEnd3, edgeStart3), nearestTimeOnEdge3));
...
float SphereOcclusionLookup(float nlAngle, float h)
{
    ...
    if (k2 > 1e-4f)
    {
        res = nl * acosf(Clamp(-nl * sqrtf((h2 - 1.0f) / Max(1.0f - nl * nl, 1e-8f)), -1.0f, 1.0f)) -
              sqrtf(k2 * (h2 - 1.0f));
...
Vector3 CapsuleVector(Vector3 capsulePosition, Quaternion capsuleRotation, float capsuleHalfLength)
{
    ...
    return Vector3Subtract(Vector3Add(capsulePosition, Vector3RotateByQuaternion(
                                                           Vector3{-capsuleHalfLength, 0.0f, 0.0f}, capsuleRotation)),
                           capsuleStart);
}
                                  ...
                                  float coneAngle)
{
    ...
    Vector3 cba =
        Vector3Subtract(Vector3Scale(Vector3Negate(coneDir), Vector3DotProduct(Vector3Negate(coneDir), ba)), ba);
